function [out] = mgngtus_cbm_mod07_modSim(parameters, subj)
...
% Pavlovian response bias, instrumental learning bias, intercept
% perseveration parameter for Avoid cues, difference parameter for Win
% cues.
% Constrain kappa and phi_Int to be positive using log1p_exp transform.
%
...
phi_Int     = log1p_exp(parameters(6));
phi_Dif     = parameters(7);
...
for t = 1:nTrial    
    ...
    if ~isnan(s)
        ...
        if la > 0 % if any last response available
             w(la) = w(la) + phi_Int + v * phi_Dif;
        end
