% mgngtus_figureS5.m
%
% Code to reproduce empirical data figures in Fig. S5A-E in the
...
% ----------------------------------------------------------------------- %
%% Fig. S5A: Bar plot p(repeat) ~ cue valence, empirical.
...
% Save source data:
fullFileName = fullfile(dirs.source, sprintf('FigS5A.csv'));
csvwrite(fullFileName, squeeze(aggrData.pStayVal(validSubs, :)));

% ----------------------------------------------------------------------- %
%% Fig. S5B-C: Bar plot p(repeat) ~ cue valence, simulated.
...
for iMod = 1:nMod
    ...
    % Save source data:
    fullFileName = fullfile(dirs.source, sprintf('FigS5%s.csv', panelIdxVec{iMod}));
    csvwrite(fullFileName, squeeze(aggrData.pStayVal(validSubs, :)));
